class BarometerGaugeCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    let host = cardConfig.host;
    let port = cardConfig.port;
    ...
    function getApiData() {
    return fetch('https://'+ip+':'+port+'/api/history/period/'+ notnowIso + '?filter_entity_id=' + cardConfig.entity + '&end_time=' + nowIso, { headers })
    ...
    getApiData().then(function(data){
        ...
        root.getElementById("recentMaxVal").innerHTML = maxval;
      	root.getElementById("svg_max_title").innerHTML = maxval;
		    root.getElementById("svg_min_title").innerHTML = minval;
    });
